...
describe("input validation", () => {
    test.each([
        ["missing http method", createEvent(undefined, VALID_HOSTNAME)],
        [
        ...
        ["unsupported method", createEvent(INVALID_METHOD, VALID_HOSTNAME)],
    ])("returns bad request error given %s", async (message, input) => {
...
describe("GET route", () => {
    describe.each([
        ...
        beforeAll(async () => {
            ...
            response = await handler(createEvent(supportedMethods.GET, url));

            dynamoDbCallsInputs = ddbMock
                .calls()
        ...
        test("returns results provided by dynamodb in ok response", async () => {
            expect(response).toBeDefined();
    ...
    test("returns internal server error if error thrown during dynamodb call", async () => {
    ...
    test("returns not found error if no results are returned from dynamodb", async () => {
        ddbMock.on(QueryCommand).resolves({
            Items: [],
        });
