...
test.each([
    [0, "no matches", "example text", "wibble"],
    [0, "no whole word matches", "exampletext", "text"],
    [1, "a single match", "example text", "text"],
    [2, "multiple matches", "example text example text", "text"],
])(
    "returns %i given %s",
    (expectedMatches: number, message: string, text: string, match: string) => {
