...
describe.each([
    ["no pathname", VALID_URL, "www.example.com.html"],
    [
        "a pathname",
...
test("throws error if content retrieval throws an error", async () => {
    ...
    expect.assertions(1);
    await expect(repository.getPageContent(VALID_URL)).rejects.toThrow(
        expectedError
    );
});
...
describe.each([
    ...
    (message: string, url: URL, expectedKey: string) => {
        ...
        beforeAll(async () => {
            ...
            response = await repository.storePageContent(url, VALID_CONTENT);
