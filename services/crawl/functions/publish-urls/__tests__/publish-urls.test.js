...
describe.each([
    ...
    beforeAll(async () => {
        ...
        const records = items.map((item) =>
            createRecord(item.baseURL, item.pathname)
        );
        ...
        entries = commands
            .map((command) => command.args[0].input.Entries)
            .flat();
    });
    ...
    test("sends all crawled pathnames and associated base URL in entry detail", () => {
        const parsedDetails = entries.map((entry) => JSON.parse(entry.Detail));
